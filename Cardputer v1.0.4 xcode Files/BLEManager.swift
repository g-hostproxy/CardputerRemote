...
struct SDNode: Identifiable, Equatable {
    var id = UUID()
    let name: String
    let path: String
...
class BLEManager: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    @Published var sdNodes: [SDNode] = []
    @Published var currentSdPath: String = "/"
    ...
    @Published var isSourAppleActive = false
    ...
    func peripheral(_ peripheral: CBPeripheral, didUpdateValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        if message.hasPrefix("SD_DIR:") {
            let dirPath = message.replacingOccurrences(of: "SD_DIR:", with: "").trimmingCharacters(in: .whitespacesAndNewlines)
            let name = (dirPath as NSString).lastPathComponent
            let node = SDNode(name: name, path: dirPath, isDirectory: true, fileSize: "")
            DispatchQueue.main.async {
                if !self.sdNodes.contains(where: { $0.path == node.path }) {
                    self.sdNodes.append(node)
                }
            }
        } else if message.hasPrefix("SD_FILE_EXT:") {
            let payload = message.replacingOccurrences(of: "SD_FILE_EXT:", with: "")
            let parts = payload.components(separatedBy: "|")
            if parts.count >= 2 {
                let filePath = parts[0].trimmingCharacters(in: .whitespacesAndNewlines)
                let fileSize = parts[1].trimmingCharacters(in: .whitespacesAndNewlines)
                let name = (filePath as NSString).lastPathComponent
                let node = SDNode(name: name, path: filePath, isDirectory: false, fileSize: fileSize)
                DispatchQueue.main.async {
                    if !self.sdNodes.contains(where: { $0.path == node.path }) {
                        self.sdNodes.append(node)
                        let lowerName = name.lowercased()
                        if (lowerName.hasSuffix(".html") || lowerName.hasSuffix(".htm")) && !self.scannedFiles.contains(name) {
                            self.scannedFiles.append(name)
                        }
                    }
                }
            }
        } else if message.hasPrefix("SD_FILE:") {
            let fileName = message.replacingOccurrences(of: "SD_FILE:", with: "").trimmingCharacters(in: .whitespacesAndNewlines)
            let name = (fileName as NSString).lastPathComponent
            let node = SDNode(name: name, path: "/" + fileName, isDirectory: false, fileSize: "--")
            DispatchQueue.main.async {
                if !self.sdNodes.contains(where: { $0.path == node.path }) {
                    self.sdNodes.append(node)
                    let lowerName = name.lowercased()
            ...
            terminal?.append("SD sync complete.", type: .success)
    ...
    func sendCommand(_ command: String) {
        ...
        if command != "PING" { terminal?.append(command, type: .tx) }
    }
    ...
    func toggleSourApple(active: Bool) {
        isSourAppleActive = active
        sendCommand(active ? "START_SOUR_APPLE" : "STOP_SOUR_APPLE")
    }
