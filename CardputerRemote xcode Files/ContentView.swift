            ...
            if !bleManager.isConnected {
                ...
                VStack(spacing: 0) {
                    ...
                    Group {
                        ...
                        case 4:
                            BluetoothExploitsView(bleManager: bleManager, themeManager: themeManager)
                        default:
                            TelemetryDashboardView(bleManager: bleManager, themeManager: themeManager)
                        }
                    }
                    ...
                    VStack(spacing: 0) {
                        ...
                        HStack(spacing: 0) {
                            ...
                            TabButton(title: "BLUETOOTH", icon: "dot.radiowaves.up.forward", index: 4, selection: $selectedTab, theme: themeManager.currentTheme)
                        }
                    }
                    ...
                    VStack(spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            VStack(spacing: 8) {
                                ...
                                SettingInfoRow(label: "Firmware Version", value: "v1.0.3")
                                SettingInfoRow(label: "BLE Link Status", value: bleManager.isConnected ? "CONNECTED" : "DISCONNECTED")
                            }
                        }
                        .padding()
                        .background(Color(red: 0.05, green: 0.05, blue: 0.05))
...
// MARK: - Bluetooth Menu & Sour Apple Exploit View
struct BluetoothExploitsView: View {
    @ObservedObject var bleManager: BLEManager
    @ObservedObject var themeManager: ThemeManager
    @State private var sourApplePulse = false
                ...
                VStack(spacing: 20) {
                    ZStack {
                        RoundedRectangle(cornerRadius: 12)
                            ...
                            .overlay(RoundedRectangle(cornerRadius: 12).stroke(bleManager.isSourAppleActive ? themeManager.currentTheme.accentColor : themeManager.currentTheme.accentColor.opacity(0.3), lineWidth: bleManager.isSourAppleActive ? 1.5 : 1))
                            .shadow(color: bleManager.isSourAppleActive ? themeManager.currentTheme.glowColor : .clear, radius: 10)

                        VStack(spacing: 8) {
                            Image(systemName: "applelogo")
                                .font(.system(size: 34))
                                .foregroundColor(bleManager.isSourAppleActive ? themeManager.currentTheme.accentColor : .gray)
                                .shadow(color: bleManager.isSourAppleActive ? themeManager.currentTheme.glowColor : .clear, radius: 6)

                            Text(bleManager.isSourAppleActive ? "SOUR APPLE SPAM ACTIVE" : "BLUETOOTH ATTACK STANDBY")
                                .font(.system(size: 10, weight: .bold, design: .monospaced))
                                .foregroundColor(bleManager.isSourAppleActive ? themeManager.currentTheme.accentColor : .white)
                        }
                    }
                    .frame(height: 170)
                    .onAppear { sourApplePulse = true }

                    VStack(spacing: 12) {
                        HStack(spacing: 15) {
                            TelemetryCard(title: "ATTACK STATUS", value: bleManager.isSourAppleActive ? "ACTIVE" : "IDLE", icon: "waveform.path.badge.plus", theme: themeManager)
                            TelemetryCard(title: "PAYLOAD", value: "iOS BLE Crash", icon: "exclamationmark.shield.fill", theme: themeManager)
                        }

                        Button(action: {
                            bleManager.toggleSourApple(active: !bleManager.isSourAppleActive)
                        }) {
                            Text(bleManager.isSourAppleActive ? "STOP SOUR APPLE" : "LAUNCH SOUR APPLE SPAM")
                                .font(.system(size: 12, weight: .bold, design: .monospaced))
                                .foregroundColor(.black)
                                .frame(maxWidth: .infinity)
                                .padding()
                                .background(bleManager.isSourAppleActive ? Color.red : themeManager.currentTheme.accentColor)
                                .cornerRadius(8)
                                .shadow(color: bleManager.isSourAppleActive ? Color.red.opacity(0.4) : themeManager.currentTheme.glowColor, radius: 6)
                        }
                    }

                    VStack(alignment: .leading, spacing: 10) {
                        HStack {
                            Text("BLUETOOTH ATTACK VECTORS")
                                .font(.system(size: 12, weight: .bold, design: .monospaced))
                                .foregroundColor(themeManager.currentTheme.accentColor)
                            Spacer()
                            Text("ESP32 BLE Spoof").font(.system(size: 9, design: .monospaced)).foregroundColor(.gray)
                        }
                        Text("Sour Apple floods rogue Apple BLE device advertisement packets to overwhelm nearby iOS UI layers.")
