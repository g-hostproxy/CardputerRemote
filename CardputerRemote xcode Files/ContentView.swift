            ...
            if !bleManager.isConnected {
                ...
                VStack(spacing: 0) {
                    HStack {
                        ...
                        Menu {
                            ForEach(CyberTheme.allCases) { theme in
                                Button(theme.rawValue) { themeManager.currentTheme = theme }
                            }
                        } label: {
                    ...
                    TabView(selection: $selectedTab) {
                        TelemetryDashboardView(bleManager: bleManager, themeManager: themeManager).tag(0)
                        ReconPortalView(bleManager: bleManager, themeManager: themeManager).tag(1)
                        WardriveView(bleManager: bleManager, themeManager: themeManager).tag(2)
                        FlockDetectorView(bleManager: bleManager, themeManager: themeManager).tag(3)
                    }
...
struct LockdownView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            Text("Awaiting encrypted BLE verification handshake with M5Stack Cardputer Advanced...")
        ...
        ScrollView {
            VStack(spacing: 20) {
                ...
                HStack(spacing: 15) {
                    ...
                    TelemetryCard(title: "STORAGE FILES", value: "\(bleManager.scannedFiles.count) Files", icon: "externaldrive.fill", theme: themeManager)
                }

                // SD Card Scanner Section
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    if bleManager.scannedFiles.isEmpty {
                        Text("No payload files synced. Tap SCAN SD.").font(.system(size: 11, design: .monospaced)).foregroundColor(.gray).padding(.vertical, 4)
                    } else {
                        ForEach(bleManager.scannedFiles, id: \.self) { file in
                            HStack {
                                Image(systemName: "doc.text.fill").foregroundColor(themeManager.currentTheme.accentColor)
                                Text(file).font(.system(size: 11, design: .monospaced)).foregroundColor(.white)
                                Spacer()
                            }
                            .padding(8)
            ...
            VStack(spacing: 20) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack(spacing: 10) {
                        ...
                        Button(action: { bleManager.sendCommand("STOP_AP") }) {
                            Text("STOP PORTAL")
                                .font(.system(size: 11, weight: .bold, design: .monospaced))
                                .foregroundColor(.white)
                                .frame(maxWidth: .infinity)
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 20) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            ...
                            HStack(spacing: 10) {
                                Button(action: {
                                    bleManager.startDeauth(bssid: network.bssid, channel: network.channel)
                                }) {
                                    Text("LAUNCH DEAUTH")
                                        .font(.system(size: 11, weight: .bold, design: .monospaced))
                                        .foregroundColor(.black)
                        ...
                        // Targeted EAPOL Handshake Sniffer Card
                        VStack(alignment: .leading, spacing: 12) {
                            HStack {
...
struct WardriveView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                // Radar Animation Box matching Theme Color
                ZStack {
                    ...
                    Circle().stroke(themeManager.currentTheme.accentColor.opacity(0.2), lineWidth: 1).frame(width: 150, height: 150)
                    Circle().stroke(themeManager.currentTheme.accentColor.opacity(0.4), lineWidth: 1).frame(width: 90, height: 90)
                    Circle().stroke(themeManager.currentTheme.accentColor.opacity(0.6), lineWidth: 1).frame(width: 40, height: 40)
                    
                    Rectangle()
                        .fill(LinearGradient(gradient: Gradient(colors: [themeManager.currentTheme.accentColor, Color.clear]), startPoint: .top, endPoint: .bottom))
                        .frame(width: 2, height: 75)
                        .offset(y: -37.5)
                        .rotationEffect(.degrees(scanAnimation ? 360 : 0))
                        .animation(bleManager.isWardrivingActive ? Animation.linear(duration: 2).repeatForever(autoreverses: false) : .default, value: scanAnimation)
                ...
                .background(Color(red: 0.05, green: 0.05, blue: 0.05))
                .cornerRadius(12)
                .overlay(RoundedRectangle(cornerRadius: 12).stroke(themeManager.currentTheme.accentColor.opacity(0.4), lineWidth: 1))
                .onAppear {
                    scanAnimation = true
                    pulseScale = 1.35
                }
...
struct FlockDetectorView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 20) {
                // High-Tech Animated Target Card matching Active Theme Color
                ZStack {
                    if bleManager.isFlockActive {
                        RoundedRectangle(cornerRadius: 16)
                            .stroke(themeManager.currentTheme.accentColor.opacity(0.4), lineWidth: 1.5)
                            .frame(height: 170)
                            .scaleEffect(pulseAnimation ? 1.03 : 0.98)
                            .opacity(pulseAnimation ? 0.3 : 1.0)
                            .animation(Animation.easeInOut(duration: 1.2).repeatForever(autoreverses: true), value: pulseAnimation)
                    }
                ...
                VStack(spacing: 12) {
                    ...
                    Button(action: {
                        bleManager.toggleFlockScanning(active: !bleManager.isFlockActive)
                    }) {
